...
export function ResetPassword() {
  ...
  const handleSubmitNewPassword = async (values: FormSchemaType) => {
    console.log("Token avant la soumission :", token);
    if (!token) {
    ...
    console.log("Token utilisé pour réinitialisation :", token);
    console.log("Nouveau mot de passe :", values.newPassword);

    try {
      setIsLoading(true);

      const result = await updatePasswordWithToken(values.newPassword);
